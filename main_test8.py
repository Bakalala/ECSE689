...
def main():
    output_dir = "generated_vhdl_test8"
    print("--- Test 6: Simple loop ---")
    ...
    # 2. Build the AST
    program = Block([
        VarStore(var_s, Cst(0)),
        For(itVar=var_i, itNum=Cst(4), body=Block([
            VarStore(var_s, Add(VarLoad(var_s), Mul(Load(RamA, VarLoad(var_i)), Load(RamB, VarLoad(var_i))))),
        ])),
    ])
